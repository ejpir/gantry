...
echo -e 'cpus\trun\trc\tcli_ms\tprep_ms\tready_ms\tonline_failures'
for C in 1 2 4 8; do
 for RUN in 1 2 3; do
  N=arm-kvm-c${C}-r${RUN}; S=/tmp/.gantry/sandboxes/$N
  ...
  BAD=$(grep -cE 'failed to online deferred CPU|CPU[0-9]+: failed|PSCI.*fail' "$S/console.log" "$S/daemon.log" 2>/dev/null)
  echo -e "$C\t$RUN\t$RC\t$CLI\t${PREP:-NA}\t${READY:-NA}\t$BAD"
  [ "$RC" -eq 0 ] && ok "arm $C-vCPU run $RUN reaches READY" || { bad "arm $C-vCPU run $RUN rc=$RC"; tail -30 /tmp/$N.start; }
  [ "$BAD" -eq 0 ] && ok "arm $C-vCPU run $RUN has no CPU-online failures" || bad "arm $C-vCPU run $RUN CPU-online failures=$BAD"
  "$G" stop "$N" >/dev/null 2>&1 || true
 done
done
# A network-enabled run creates the guest /etc/hosts required by this older
# vminitd rootfs; use it only for post-READY workload CPU visibility.
N=arm-kvm-online8; S=/tmp/.gantry/sandboxes/$N
"$G" stop "$N" >/dev/null 2>&1 || true; rm -rf "$S"
...
RC=$?; [ "$RC" -eq 0 ] && ok 'arm network-enabled 8-vCPU guest reaches READY' || bad "arm network-enabled 8-vCPU rc=$RC"
sleep 2
