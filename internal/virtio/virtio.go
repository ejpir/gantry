...
// RaiseExternalUsedInterrupt publishes a completion generated by an external
// vhost-style device worker. The worker writes the used ring in shared guest
// RAM first, then signals its call doorbell; this method supplies the ordinary
// virtio-mmio interrupt-status and host IRQ half of that protocol.
func (c *Core) RaiseExternalUsedInterrupt() {
	c.mu.Lock()
	c.raiseIRQ(virtioIntUsedBuffer)
	c.mu.Unlock()
}
