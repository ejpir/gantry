...
func TestDecodeX86MMIO(t *testing.T) {
	cases := []struct {
		...
		{"movq $0x80000001,(%rax)", []byte{0x48, 0xc7, 0x00, 0x01, 0x00, 0x00, 0x80}, MMIOOp{IsWrite: true, Width: 8, ImmOK: true, Imm: 0xffffffff80000001, Reg: -1, Length: 7}},
		{"movw $0x1234,(%rax)", []byte{0x66, 0xc7, 0x00, 0x34, 0x12}, MMIOOp{IsWrite: true, Width: 2, ImmOK: true, Imm: 0x1234, Reg: -1, Length: 5}},
		{"movq $2,(%rax) with 66h; REX.W wins", []byte{0x66, 0x48, 0xc7, 0x00, 0x02, 0x00, 0x00, 0x00}, MMIOOp{IsWrite: true, Width: 8, ImmOK: true, Imm: 2, Reg: -1, Length: 8}},
