...
func validateShareTarget(share shares.Spec) error {
	ctr := share.CtrPath
	if ctr == "" {
		ctr = config.DefaultHubCtrPath(share.Tag)
	}
	if config.ContainerPathsOverlap(ctr, shares.HubInternalPath) {
...
func (m *ShareManager) completeIdenticalAddLocked(existing *managedShare, persistent bool) (shares.Entry, error) {
	...
	persistErr := m.mutateSharesSnapshotLocked(existing.share.Tag, config.ShareConfigSpec(existing.share), &oldConfig)
...
func (tx *shareAddTransaction) persist() error {
	...
	tx.persistErr = tx.manager.mutateSharesSnapshotLocked(
		...
		config.ShareConfigSpec(tx.addition.share),
		&tx.oldConfig,
	)
...
func (m *ShareManager) removeLocked(tag string, persistent, force bool) (shares.Entry, error) {
	...
	if ctr == "" {
		ctr = config.DefaultHubCtrPath(tag)
	}
	if ctr != config.DefaultHubCtrPath(tag) && !force {
...
func (m *ShareManager) entry(ms *managedShare) shares.Entry {
	ctr := ms.share.CtrPath
	if ctr == "" {
		ctr = config.DefaultHubCtrPath(ms.share.Tag)
	}
	state := "active"
	if ms.export != nil {
...
func (m *ShareManager) mutateSharesSnapshotLocked(tag, newSpec string, old *[]string) error {
	return m.store.Mutate(func(cfg *config.RunConfig) error {
		*old = append([]string(nil), cfg.Shares...)
		cfg.Shares = config.ShareSpecsReplacingTag(cfg.Shares, tag, newSpec)
		return nil
	})
}
