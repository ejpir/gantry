...
func (w *Worker) Done() <-chan struct{} { return w.lifecycle.Done() }
...
func (w *Worker) StartTrafficSync(recorder *netpol.TrafficRecorder) {
	w.StartTrafficSyncEvery(recorder, networkWorkerTrafficSyncInterval)
}

func (w *Worker) StartTrafficSyncEvery(recorder *netpol.TrafficRecorder, interval time.Duration) {
...
func (w *Worker) syncTraffic(ctx context.Context, interval time.Duration) {
...
func (w *Worker) mergeFinalTraffic(snapshot *netpol.TrafficSnapshot) {
	w.stopTrafficSync()
...
// Start spawns the worker process, performs the handshake and
...
func (w *Worker) SetPolicy(policy *netpol.Policy) error {
...
func (w *Worker) syncPolicyState() error {
...
func (w *Worker) readPolicyStatus(transaction string) (networkworker.PolicyStatusResponse, error) {
...
func (w *Worker) commitPolicyTransaction(gen uint64, txn string) error {
...
func (w *Worker) stopAfterAmbiguousPolicy(cause error) error {
	stopErr := w.Close()
	return errors.Join(cause, fmt.Errorf("network policy state ambiguous; network worker stopped"), stopErr)
}

func (w *Worker) Publish(proto, local, remote string) error {
...
func (w *Worker) Unpublish(proto, local string) error {
	txn, err := newWorkerTransactionID()
	if err != nil {
		return fmt.Errorf("port transaction nonce: %w", err)
	}
...
func (w *Worker) applyPortMutation(op, transaction string, request any, forward vnet.Forward, present bool) error {
	w.portMu.Lock()
...
func (w *Worker) readPortStatus(transaction string) (networkworker.PortStatusResponse, error) {
...
func (w *Worker) TrafficSnapshot() (netpol.TrafficSnapshot, error) {
	var out netpol.TrafficSnapshot
	err := w.client.Call(networkworker.OpTrafficSnapshot, nil, &out)
	return out, err
}

func (w *Worker) trafficSnapshotContext(ctx context.Context) (netpol.TrafficSnapshot, error) {
	var out netpol.TrafficSnapshot
	err := w.client.CallContext(ctx, networkworker.OpTrafficSnapshot, nil, &out)
	return out, err
}

func (w *Worker) Capture(request packetcapture.Request) (packetcapture.Snapshot, error) {
...
// TrafficDebug mirrors GANTRY_DEBUG_NET into the bootstrap config.
func TrafficDebug() bool { return os.Getenv("GANTRY_DEBUG_NET") != "" }

// Attach wraps a worker whose control and data channels the caller already
// established, rather than spawning one as Start does. The supervisor takes
// the same ownership either way: Close shuts the worker down and releases
// both channels.
func Attach(control, data net.Conn) *Worker {
	return &Worker{
		client:    workerproto.NewClient(control),
		data:      data,
