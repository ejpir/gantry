...
func CmdNetworkPolicy(argv []string) int {
  ...
  gantry net-policy show <name>`)
	...
	case "set":
		...
		entry, err := SetNetworkPolicy(args[0], args[1], allowLocal)
		if err != nil {
	...
	case "default", "clear":
		...
		entry, err := SetNetworkPolicy(args[0], "", allowLocal)
		if err != nil {
	...
	case "show":
		...
		entry, err := GetNetworkPolicy(args[0])
		if err != nil {
...
func networkPolicyRPC(name, op string, request *controlproto.NetworkPolicyRequest) (control.NetworkPolicyEntry, error) {
	req := controlproto.Request{Op: op, ID: controlproto.NewRequestID("netpolicy"), NetPolicy: request}
	resp, err := controlproto.Call[controlproto.NetworkPolicyResponse](name, req)
	if err != nil {
	...
	if !resp.OK || resp.Policy == nil {
		...
		return control.NetworkPolicyEntry{}, fmt.Errorf("%s", resp.Error)
	}
	return *resp.Policy, nil
}

func GetNetworkPolicy(name string) (control.NetworkPolicyEntry, error) {
	...
	cfg := store.Snapshot()
	path, policy, err := control.ResolveNetworkPolicy(cfg.NetPol, cfg.AllowLN)
	if err != nil {
		return control.NetworkPolicyEntry{}, err
	}
...
func hydrateNetworkPolicyRules(entry control.NetworkPolicyEntry) control.NetworkPolicyEntry {
	if len(entry.Rules) != 0 {
		return entry
	}
	_, policy, err := control.ResolveNetworkPolicy(entry.Path, entry.AllowLocal)
	if err == nil {
...
func printNetworkPolicyMutation(entry control.NetworkPolicyEntry) {
	path := entry.Path
	if path == "" {
