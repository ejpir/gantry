// Package boundedlog is the bounded log pipe: an os.Pipe whose read end is
// drained at a capped rate into a size-bounded, self-compacting file. It
// exists so an attacker-influenced writer — a guest serial console, a worker
// process's stderr — can never fill the disk or block the writer that feeds
// it.
package boundedlog
...
// Pipe is a one-way capability: an untrusted producer receives
...
func (p *Pipe) drain(sink io.WriteCloser) {
...
func newSink(path string) (io.WriteCloser, error) {
	if path == "" {
	...
	sink := &sink{file: file, size: info.Size()}
...
// RotatePrevious moves an existing log aside to <path>.previous so one run's
// output survives the next run truncating the file. A missing log is not an
// error: the first run has nothing to rotate.
func RotatePrevious(path string) error {
