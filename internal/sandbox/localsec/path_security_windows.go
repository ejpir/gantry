...
// CreateDir protects the root before creating its child. That
...
func secureWindowsPath(path string, directory, inheritChildren bool) error {
	...
	if err := VerifyPrivate(path, userSID, inheritChildren); err == nil {
		return nil
	}
...
func VerifyPrivate(path string, userSID *windows.SID, inheritChildren bool) error {
	descriptor, err := windows.GetNamedSecurityInfo(
		path,
		windows.SE_FILE_OBJECT,
