...
func (dashboardService) DefaultShareMount(tag string) string { return config.DefaultHubCtrPath(tag) }
...
func (dashboardService) ValidateResources(memMB uint, vcpus int, processIsolation string) error {
	...
	return config.ValidateProcessIsolation(processIsolation)
}
...
func (dashboardService) ValidateNetworkPolicy(path string, allowLocal bool) error {
	_, _, err := control.ResolveNetworkPolicy(path, allowLocal)
	return err
}

func (dashboardService) SetNetworkPolicy(name, path string, allowLocal bool) (dashboardapi.PolicyResult, error) {
	entry, err := controlcmd.SetNetworkPolicy(name, path, allowLocal)
...
func (dashboardService) ValidateSecret(request dashboardapi.SecretRequest) error {
	...
	if len(request.Value) > controlproto.SecretsHandshakeMaxBytes {
		return dashboardapi.Invalid("value", fmt.Errorf("secret value is too large"))
	}
	wireProbe := controlproto.Request{
		Op: "secret.set", ID: strings.Repeat("x", 64),
		Secret: &controlproto.SecretRequest{Name: strings.TrimSpace(request.Name), Value: request.Value},
	}
	wire, err := json.Marshal(wireProbe)
	if err != nil || len(wire)+1 > controlproto.MaxRequestBytes {
		return dashboardapi.Invalid("value", fmt.Errorf("secret value is too large for a live control request"))
	}
	if _, alive := layout.PID(request.Sandbox); !alive {
...
func (dashboardService) RemoveSecret(row dashboardapi.Secret) error {
	if err := layout.ValidateName(row.Sandbox); err != nil {
		return err
	}
...
func mutateDashboardNetworkPolicy(name string, mutate func(*netpol.Policy) (*netpol.Policy, error)) error {
	...
	_, policy, err := control.ResolveNetworkPolicy(cfg.NetPol, cfg.AllowLN)
	if err != nil {
		return err
	}
	...
	dir := filepath.Join(layout.Dir(name), "network-policies")
	...
	_, err = controlcmd.SetNetworkPolicy(name, path, cfg.AllowLN)
	return err
}
...
func (dashboardService) ConfigureShare(plan dashboardapi.SharePlan) error {
	return controlcmd.ConfigureShare(plan.Sandbox, plan.Spec, plan.Replace)
}

func (dashboardService) RemoveShare(mount dashboardapi.Mount) error {
	return controlcmd.RemoveShare(mount.Sandbox, mount.Tag, dashboardShareRemovalNeedsForce(mount))
}
...
func (dashboardService) PlanPort(req dashboardapi.PortRequest) (string, error) {
	...
	if _, err := config.ParsePortSpec(spec); err != nil {
		return "", dashboardapi.Invalid("bind", err)
	}
	return spec, nil
}
...
func loadDashboardSnapshot() (dashboardapi.Snapshot, error) {
	...
	for _, entry := range entries {
		...
		if raw, readErr := os.ReadFile(sandbox.ConfigPath); readErr == nil && json.Unmarshal(raw, &cfg) == nil {
			...
			sandbox.ProcessIsolation = config.NormalizeProcessIsolation(cfg.ProcessIsolation)
			...
			if sandbox.Proxy != "" && sandbox.NoProxy == "" {
				sandbox.NoProxy = config.DefaultNoProxy
			}
...
func loadDashboardRules(sandbox string, cfg config.RunConfig) []dashboardapi.Rule {
	if !cfg.Net {
	...
	if proxy, err := config.ParseForwardProxy(cfg.ProxyURL); err == nil && proxy.URL != "" {
...
func loadDashboardMounts(sandbox string, cfg config.RunConfig, running bool) ([]dashboardapi.Mount, bool) {
	...
	rowForConfigured := func(share shares.Spec, state string) dashboardapi.Mount {
		return dashboardapi.Mount{
			...
			VM: shares.HubVMPath + "/" + share.Tag, Guest: config.ConfiguredShareTarget(share),
			ReadOnly: share.RO, UID: share.UID, GID: share.GID, State: state,
		}
	}
	if running {
		if raw, err := os.ReadFile(filepath.Join(layout.Dir(sandbox), "shares.json")); err == nil {
			...
			if json.Unmarshal(raw, &manifest) == nil && manifest.Transport != nil {
				...
				for _, entry := range manifest.Shares {
					row := dashboardapi.Mount{
						...
						State: controlcmd.DefaultShareState(entry.State),
					}
					if row.State == "error" {
...
func loadDashboardPorts(sandbox string, cfg config.RunConfig, running bool) []dashboardapi.Port {
	rowFor := func(mapping config.PortMapping, state string) dashboardapi.Port {
	...
	if running {
		resp, err := controlcmd.PortRPC(sandbox, "port.list", controlproto.PortRequest{Persistent: true})
		if err != nil {
