...
func TestDashboardSnapshotLoadsSandboxData(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(layout.Dir("alpha"), netpol.TrafficFileName), trafficJSON, 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestDashboardMountsMergeLiveAndDesiredState(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(layout.Dir(name), "shares.json"), []byte(manifest), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestDashboardMountsPreserveLiveErrorDuringDrift(t *testing.T) {
	...
	rows, live := loadDashboardMounts(name, config.RunConfig{Shares: []string{"code=/new"}}, true)
...
func TestDashboardTrafficRulesPersistForAllProtocols(t *testing.T) {
	...
	writeDashboardTestConfig(t, name, config.RunConfig{Net: true, MemMB: 512, VCPUs: 1})
	service := dashboardService{}
...
func TestDashboardBlockedDNSAddsDomainAllowlistEntry(t *testing.T) {
	...
	writeDashboardTestConfig(t, name, config.RunConfig{Net: true, MemMB: 512, VCPUs: 1, NetPol: policyPath})
...
func writeDashboardTestConfig(t *testing.T, name string, cfg config.RunConfig) {
	t.Helper()
	raw, err := json.Marshal(cfg)
	if err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(layout.Dir(name), "sandbox.json"), raw, 0o600); err != nil {
		t.Fatal(err)
	}
}
