...
#
# Needs: mkfs.erofs, fsck.erofs, curl. Downloads the matching runsc
# release binary (static, ~45 MB) from the gVisor release bucket.
set -e

IN=${1:?"usage: mkrootfs-gvisor.sh nerdbox-rootfs-ARCH.erofs"}
case "$IN" in
*arm64*|*aarch64*) RUNSC_ARCH=aarch64 ;;
...
echo "== installing runsc as /sbin/crun (crun -> /sbin/crun.runc)"
mv "$WORK/rootfs/sbin/crun" "$WORK/rootfs/sbin/crun.runc"
cp "$WORK/runsc" "$WORK/rootfs/sbin/crun"
