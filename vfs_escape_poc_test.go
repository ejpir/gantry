...
// bytes on the virtio-fs request virtqueue. This test drives virtioFS.handler
...
func fuseInitDevice(t *testing.T, dev *virtioFS) {
	t.Helper()
...
func lookup(t *testing.T, dev *virtioFS, unique uint64, parent uint64, name string) (uint64, int32) {
	t.Helper()
...
func req(t *testing.T, dev *virtioFS, in [][]byte, outSizes ...int) (int, int32, [][]byte) {
	t.Helper()
...
func TestVirtioFSShareEscape(t *testing.T) {
	...
	dev, err := newVirtioFS("escape", share)
	if err != nil {
		t.Fatal(err)
	}
...
func TestVirtioFSSymlinkEscapeBlocked(t *testing.T) {
	...
	dev, err := newVirtioFS("symlink-escape", share)
	if err != nil {
		t.Fatal(err)
	}
...
func TestVirtioFSReadOnlyShare(t *testing.T) {
	...
	dev, err := newVirtioFS("ro-share", share, true)
	if err != nil {
		t.Fatal(err)
	}
	fuseInitDevice(t, dev)
