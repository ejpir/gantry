...
func setupQueue(mem guestMem, core *virtioMMIOCore, qn, num uint32) {
...
func putDesc(mem guestMem, qn uint32, idx uint16, addr uint64, length uint32, flags uint16, next uint16) {
	var d [16]byte
...
func availPush(mem guestMem, qn uint32, head uint16) {
...
func usedPop(mem guestMem, qn uint32) (usedElem, bool) {
...
func TestVirtioBlkRead(t *testing.T) {
	...
	hdr := make([]byte, 16)
	binary.LittleEndian.PutUint32(hdr[0:], virtioBlkTIn)
...
func TestVirtioFSLoopbackProtocol(t *testing.T) {
	...
	dev, err := newVirtioFS("testshare", root)
	if err != nil {
		t.Fatal(err)
	}
...
func TestVirtioBlkWrite(t *testing.T) {
	...
	blk, err := newVirtioBlk(img, true)
	if err != nil {
		t.Fatal(err)
	}
	defer blk.file.Close()
	if f := blk.features(); f&(1<<virtioBlkFRO) != 0 || f&(1<<virtioBlkFFlush) == 0 {
	...
	hdr := make([]byte, 16)
	binary.LittleEndian.PutUint32(hdr[0:], virtioBlkTOut)
	...
	binary.LittleEndian.PutUint32(hdr[0:], virtioBlkTFlush)
	mem.writeAt(ramBase+testDataAddr, hdr)
	...
	roBlk, err := newVirtioBlk(img, false)
	if err != nil {
		t.Fatal(err)
	}
	defer roBlk.file.Close()
	if roBlk.features()&(1<<virtioBlkFRO) == 0 {
...
func TestVirtioRTC(t *testing.T) {
	...
	roundTrip := func(req []byte) (status byte, body []byte) {
		...
		putDesc(mem, 0, 1, ramBase+testDataAddr+0x100, virtioRTCRespLen, vringDescFWrite, 0)
		availPush(mem, 0, 0)
		core.mmioWrite(0x050, 0)
		e, ok := usedPop(mem, 0)
		if !ok || e.len != virtioRTCRespLen {
			t.Fatalf("used elem = %+v ok=%v", e, ok)
		}
		out := make([]byte, virtioRTCRespLen)
	...
	if st != virtioRTCSOK || binary.LittleEndian.Uint16(body) != 1 {
	...
	if st != virtioRTCSOK || binary.LittleEndian.Uint64(body) != want {
...
func TestVirtioNetUnixgramVFKIT(t *testing.T) {
	...
	nic, err := newVirtioNetUnixgram(path, [6]byte{2, 0, 0, 0, 0, 1}, true)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestVirtioVsockHandshakeAndRW(t *testing.T) {
	...
	if !irqs.raised[virtioMMIOIRQ+1] {
