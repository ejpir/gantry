...
const (
	...
	virtioMMIOIRQ    = 48 // INTID of SPI 16
...
// guestMem is the device's view of guest physical memory.
...
// virtioDevice is implemented by each emulated device (blk, vsock).
...
// virtioMMIOCore is the transport state machine shared by all devices.
type virtioMMIOCore struct {
	mu      sync.Mutex
	dev     virtioDevice
	mem     guestMem
	irq     int
...
// newVirtioMMIOAt attaches dev at an explicit MMIO base and IRQ (the
// machine picks arch-appropriate values; see machine.addVirtio).
func newVirtioMMIOAt(dev virtioDevice, mem guestMem, base uint64, irq int, irqLine func(int, bool), name string) *virtioMMIOCore {
	return &virtioMMIOCore{
		dev:     dev,
		mem:     mem,
		irq:     irq,
		irqLine: irqLine,
...
func (c *virtioMMIOCore) mmioRead(off uint64, length uint32) uint32 {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *virtioMMIOCore) mmioWrite(off uint64, val uint32) {
	c.mu.Lock()
	defer c.mu.Unlock()
	switch {
	case off >= 0x100:
...
func (c *virtioMMIOCore) descAt(q *virtq, i uint16) (desc, error) {
	var d desc
	var buf [16]byte
...
func (c *virtioMMIOCore) raiseIRQ(bit uint32) {
	c.isr |= bit
...
func (c *virtioMMIOCore) interruptSuppressed(q *virtq) bool {
	var flags [2]byte
...
func (c *virtioMMIOCore) readChains(ds []desc) ([]byte, error) {
	var out []byte
	for _, d := range ds {
		buf := make([]byte, d.len)
...
func (c *virtioMMIOCore) writeChains(ds []desc, data []byte) (uint32, error) {
	var n uint32
	for _, d := range ds {
