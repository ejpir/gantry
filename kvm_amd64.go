...
func (b *kvmX86Backend) runVCPULoop(vc *kvmVCPU) error {
	...
	for {
		...
		case kvmExitFailEntry:
			return fmt.Errorf("KVM_EXIT_FAIL_ENTRY hardware_reason=%d (vmx instr error)", le64(vc.run.data[16:]))
		case kvmExitInternalError:
			return fmt.Errorf("KVM_EXIT_INTERNAL_ERROR suberror=%d", le32(vc.run.data[32:]))
		default:
